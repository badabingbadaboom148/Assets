...
public class AntiMissileGunScanner : MonoBehaviour
{
        ...
        for (int i = 0; i < targetsInViewRadius.Length; i++)
        {
            ...
            if (IsTargetValid(targetPosition))
            {
                ...
                if (teamController != null)
                {
                    ...
                    else if (!teamController.isSpotted || teamController.isFriendly == transform.root.GetComponent<TeamController>().isFriendly)
                    {
    ...
    private void SelectClosestTarget()
    {
        ...
        for (int i = 0; i < targetList.Count; i++)
        {
            ...
            if (currentTargets == null || distance < closestDistance)
            {
