...
public class AntiMissileGunController : MonoBehaviour {
	...
	private void LeadTarget()
	{
		...
		Vector3 aimPoint = target.position + target.GetComponent<Rigidbody>().velocity * 0.75f * projectileTravelTime;
		...
		predictedTargetPosition = aimPoint - 0 * Physics.gravity * projectileTravelTime * projectileTravelTime * -1;
	...
	protected virtual void Fire()
	{
		...
		if (target != null && ProjectileCount > 0)
		{
			for (int i = 0; i < Barrel.Length; i++)
			{
				if (Time.time > nextFireAllowed && IsAiming)
				{
					if (UsePooling)
				...
				if (audioSource != null && firingSFX != null && !audioSourcePlaying)
				{
			...
			if (BulletShellFX != null)
			{
				...
				Invoke("StopBulletShellEffect", 1.2f); // This line seems to stop the effect after 1.2 seconds. Ensure it's what you want.
			}
